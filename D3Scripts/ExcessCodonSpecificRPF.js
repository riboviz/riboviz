//Width and height
			var marginFigure6 = {top: 20, right: 20, bottom: 20, left: 30},
    		...
    		//var bisectposition = d3.bisector(function(d) { return d.Codon; }).right;
			var colorFigure6 = d3.scale.category20()
					.domain(["Data", "CHX", "non-CHX"])
    		...
    		var xFigure6 = d3.scale.ordinal()
    					.rangeRoundBands([paddingFigure6, widthFigure6 - paddingFigure6 * 2], .1);
    						//.range([paddingFigure6, widthFigure6 - paddingFigure6 * 2]);
			...
			//Load data
			d3.tsv("../VizData/F6_Temp.tsv", function(error, data) {
...
function updateFigure6(data, value1, value2, value3) {
      				
var datanew = data.filter(function(d, key) { 
            return ((d.Year==value1   | d.Year=="All") & (d.Author==value2   | d.Author=="All") & (d.Dataset==value3   | d.Dataset=="All") );
...
//console.log(datanew);   
				...
				legendSpace6 =  widthFigure6/nucleotide.length;
  				colorFigure6.domain(["Data", "CHX", "non-CHX"]);
...
var lineSvg6=freqFigure6.enter().append("g").attr("class", "nucleotide");
      ...
      lineSvg6.append("text")
				...
				.attr("transform", function(d, i) { return "translate(" + ((widthFigure6-paddingFigure6l*11/10)) + "," + ((legendSpace6/15)+i*legendSpace6/15)+ ")"; })
                ...
                if (name.name=="non-CHX"){d3.select("#tagnon-CHX")
                     .transition().duration(100) 
                     .style("opacity", newOpacity);} 
...
function change6() {
	...
	updateFigure6(data, year6,author6, thedataset6);
};
