...
var marginFigure8 = {top: 60, right: 60, bottom: 20, left: 50},
		paddingFigure8=5,
    	widthFigure8 = 600 - marginFigure8.left - marginFigure8.right,
...
function change8() {
	...
	console.log(thedataset8);
	var string="../../VizData/";
	console.log(string);
	var thefile=string.concat("F8_Temp_Year_",year8,"_Author_",author8, "_Dataset_", thedataset8,"_data.tsv");
	console.log(thefile);
...
var  data = data.map( function (d) { 
	...
	if(value1=="mRNA"){choosetheone=d.mRNA};
	//if(value1=="IniProb"){choosetheone=d.IniProb};
	//if(value1=="IniEff"){choosetheone=d.IniEff};
	//if(value1=="SynRates"){choosetheone=d.SynRates};
...
textFigure8.exit().attr("class", "exit")
	...
	// svgFigure8.select(".r-line")			
// 			.attr("x1", xFigure8(trendData[0][0]))
//   			.attr("y1", yFigure8(trendData[0][1]))
//   			.attr("x2",  xFigure8(trendData[0][2]))
//   			.attr("y2", yFigure8(trendData[0][3]))
//   			.attr("stroke", "green")
//  			.style("stroke-width",0.9)
// 			.transition()
// 			.ease("linear")
...
function changeit8() {
  	...
  	var testabundance= d3.select('input[name="reads"]:checked').node().value;
    var testsite= d3.select('input[name="features"]:checked').node().value;
    updateFigure8(data, testabundance, testsite);
