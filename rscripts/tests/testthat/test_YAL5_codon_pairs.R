...
#'
#' These tests run `YAL5-codon-pairs.R`.
#'
...
#'
#' ```
#' test_file("rscripts/tests/testthat/test_YAL5_codon_pairs.R")
#' ```
#'
#' To run the tests from the console:
#'
#' ```
#' Rscript -e 'library(testthat); test_file("rscripts/tests/testthat/test_YAL5_codon_pairs.R")'
#' ```
#'
...
context("test_YAL5_codon_pairs.R")

#' Run `YAL5-codon-pairs.R`.
#'
#' @param yal5_codon_pairs `YAL5-codon-pairs.R` path (character).
...
#'
#' @export
RunYal5CodonPairs <- function(yal5_codon_pairs, h5_file, dataset,
  gff_file, asite_file, annotation_file, feature, output_dir,
  exit_code = 0) {

  cmd_template <- "Rscript --vanilla {yal5_codon_pairs} --input={h5_file} --dataset={dataset} --gff={gff_file} --asite_length={asite_file} --annotation={annotation_file} --feature='{feature}' --output={output_dir}"
...
testthat::test_that("--feature codon", {
  ...
  withr::with_tempdir({
    ...
    output_file <- "Meta_feature_plotCCA TGG.pdf"
...
testthat::test_that("--feature codon", {
  ...
  withr::with_tempdir({
    ...
    output_file <- "Feature_Relative_use.tsv"
    output_path <- file.path(".", output_file)
    testthat::expect_true(file.exists(output_path),
         info = paste(output_path, "does not exist"))
  })
