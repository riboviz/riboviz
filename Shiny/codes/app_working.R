...
df<-read.table('data/data_unq.tsv') #stores the year/author/database information
   ...
   observeEvent(
    input$clickplot,{ #store all the intermediate reactive values needed for making the plot
      ...
      for (i in 1:length(temp())){
        ...
        state$inf[i]<-paste('/Users/xtj/Documents/shahlab/shinyProject/data/riboseq/',temp()[i],'/',state$geoid[i],'.h5',sep='')
