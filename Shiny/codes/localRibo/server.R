...
source('all_sessions.R', local=TRUE) # Objects in this file are shared across all sessions

shinyServer<-function(input,output){
  #path<-'/Users/xtj/Documents/shahlab/shinyProject/localRibo'
  df<-read.table(paste0(path,'/dataList/data_unq.tsv')) #stores the year/author/database information
  names(df)=c('year','author','NA','NA','database','NA','NA','NA','dbtype','condition','geoID')
  ...
  rpkmpath<-paste0(path,'/dataList/')
  d1<-read.delim(paste0(rpkmpath,'localRPKMs.tsv'))
      ...
      if (length(temp())==0){ #check if database selection is empty
        ...
        if(input$txt %in% names(d2)){ #check if gene name is correct
          ...
          for (i in 1:length(temp())){
            ...
            state$inf[i]<-paste0(path,'/data/riboseq/',temp()[i],'/',state$geoid[i],'.h5')
