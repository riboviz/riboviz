...
shinyUI<-fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput(inputId = 'selectyear',label='Select the year of publication',choices=c('Choose',2009,2012,2013,2014,2015,2016)),
    ...
    input$clickplot,{ #store all the intermediate reactive values needed for making the plot
        ...
        isolate({
          if(input$select=='rbl'){
            ...
            ggplot(dat, aes(x = x, y=value, fill = variable)) +
              ...
              scale_fill_manual(values=c(rgb(166,206,227,maxColorValue=255),rgb(239,138,98,maxColorValue=255),'green'))
          }
      ...
      for (i in 1:length(state$temp)) {
        ...
        local({
          ...
          #print(state$attrdat[(36*(my_i-1)+1):(36*my_i)])
