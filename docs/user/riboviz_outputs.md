...
File with the transcripts per million (tpm) for all successfully processed samples. Generated by collate_TPMs.R. This script uses the tpms.tsv file for each processed sample to list the tmps for each gene, allowing for comparison between samples.
