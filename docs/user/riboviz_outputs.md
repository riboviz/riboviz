...
A tsv file showing the transcripts per million, a feature (ie Length_log10, FE_atg, uATGs etc) and a value for that feature. This is produced by combining the tpms file with the features file, if a features file is provided. This is done by generate_stats_figs.R during step "Correlations between TPMs of genes with their sequence-based features" using function CalculateSequenceBasedFeatures then WriteSequenceBasedFeatures.
...
The features pdf relates the transcripts per million value of different genes to a variety of different sequence features. This highlights any trends in feature value as transcripts per million changes. Values for different the features come from the features-file, if it is provided.
