...
| `dir_tmp` | Temporary directory for intermediate files |
| `dir_logs` | Log files directory |
| `cmd_file` | File to log bash commands invoked by the workflow |
| `rRNA_fasta` | Ribosomal rRNA and other contaminant sequences to avoid aligning to (FASTA file) |
| `orf_fasta` | Transcript sequences file containing both coding regions and flanking regions (FASTA file) |
...
| `dir_index` | Directory to put built index files into |
| `rRNA_index` | Prefix for rRNA index files, relative to `<dir_index>` |
| `orf_index` | Prefix for ORF index files, relative to `<dir_index>` |
...
| `adapters` | Illumina sequencing adapter to remove |
...
| `nprocesses` | Number of processes to parallelize over, used by specific steps in the workflow |
...
| `MinReadLen` | Minimum read length in H5 output |
| `MaxReadLen` | Maximum read length in H5 output |
| `Buffer` | Length of flanking region around the CDS |
| `PrimaryID` | Primary gene IDs to access the data (YAL001C, YAL003W, etc.) |
| `SecondID` | Secondary gene IDs to access the data (COX1, EFB1, etc. or `NULL`) |
| `dataset` | Human-readable name of the dataset |
| `StopInCDS` | `TRUE|FALSE`, are stop codons part of the CDS annotations in GFF? |
| `isTestRun` | `TRUE|FALSE`, is this a test run? (unused) |
| `rpf` | `TRUE|FALSE`, is the dataset an RPF or mRNA dataset? |
| `ribovizGFF` | `TRUE|FALSE`, does the GFF file contain 3 elements per gene - UTR5, CDS, and UTR3? |
...
| `t_rna` | tRNA estimates file (tab-separated values file) |
| `codon_pos` | Position of codons within each gene (RData file) |
