...
I tested whether the source("script.R", local=knitr::knit_global()) or sys.source("script.R, envir=knitr::knit_global()) block described [here in the rmarkdown cookbook](https://bookdown.org/yihui/rmarkdown-cookbook/source-script.html) would help instead of the repeated x <- params$x, but **it doesn't work**, and means that the default parameters from the yaml are used in the external code blocks :C

```{r echo=FALSE}
knitr::read_chunk(here("rmarkdown","ParamsTestScript.R"))
```

```{r paramslol, echo=FALSE}
<<intialargs_params>>
paste("ParamsTest.Rmd sees min_read_length as", as.character(min_read_length))
paste("ParamsTest.Rmd sees min_read_length as", as.character(max_read_length))
```
Repeat the parameter stuff to see what it reads as now we've called in blocks from ParamsTestScript.R:
```{r color2}
...
Extra parameters: what do they show up as?
```{r theend}
params$extraParam
params$extraParam2
```
