...
### Prepare Annotations and Sequences
```{r}
# Download annotations and genome seq from here - 
...
ecoli_annot <- readGFFAsGRanges("~/ecoliK12_refseq.gff")
ecoli_genome <- readDNAStringSet("~/ecoliK12.fasta")

# Pad genomes with 25 N nts to provide null flanking regions for genes close to chromosomal boundaries
ecoli_genome <- xscat(DNAString(paste(rep("N",25),collapse="")),
                      ecoli_genome,
                      DNAString(paste(rep("N",25),collapse="")))

names(ecoli_genome) <- c("NC_000913.3")

# Adjust the annotations for this 250nt shift
ecoli_annot <- shift(ecoli_annot,25)

# Get the annotations for all CDS of uncharacterized and verified genes
cds_annot <- ecoli_annot[ecoli_annot$type=="CDS"]
...
tmp_gff <- sapply(unique(cds_annot$Name),
                  function(x){
                    gene_location <- cds_annot[which(cds_annot$Name==x),]; 
                    resize_gff_flank(gene_location = gene_location, flank=25)
                    })
cds_flank_annot <- unlist(GRangesList(tmp_gff))

# Obtain sequences of CDS+UTRs from genome
cds_flank_seq <- ecoli_genome[cds_flank_annot]
names(cds_flank_seq) <- cds_flank_annot$Name
...
# Write seqs to a FASTA file
writeXStringSet(output_seq,filepath = "~/ecoli_CDS_w_25utrs.fa",format = "fasta")
```

### Create a GFF file based on these transcripts
```{r, eval=FALSE}
len_gff <- c(matrix(c(rep(25,length(output_seq)), (width(output_seq)-50), rep(25,length(output_seq))),nrow=3,byrow = T))
start_gff <- c(matrix(c(rep(1,length(output_seq)),rep(26,length(output_seq)), (width(output_seq)-24)),nrow=3,byrow = T))
type_gff <- rep(c("UTR5","CDS","UTR3"),length(output_seq))
...
output_gff <- GRanges(seqnames = rep(names(output_seq),each=3), 
                      ...
                      strand = "+",type=type_gff,
                      Name=rep(names(output_seq),each=3))
export.gff3(output_gff, con="~/ecoli_CDS_w_25utrs.gff3")
```
...
cods <- sapply(sapply(seq,Biostrings::codons),as.character) # Split the sequences into codons
...
save(codon_pos, file = "~/ecoli_codon_pos_i0.RData")
```
...
nt_seq <- strsplit(as.character(seq),"")
seq <- seq[gff[gff$type=="CDS"]] # Restrict sequences to only CDS
cod_seq <- sapply(sapply(seq,Biostrings::codons),as.character) # Split the sequences into codons
