...
def parse_args(args=None):
    ...
    Epilog = 'Example usage: python fetch_sra_runinfo.py <ID_IN> <FILE_OUT>'

    parser = argparse.ArgumentParser(description=Description, epilog=Epilog)
    parser.add_argument('ID_IN', help="File containing database identifiers, one per line.")
...
def fetch_sra_runinfo(id_in,file_out,platform_list=[],library_layout_list=[]):
    total_out = 0
    seen_ids = []; run_ids = []
    header = []
...
def main(args=None):
    ...
    fetch_sra_runinfo(args.ID_IN,args.FILE_OUT,platform_list,library_layout_list)
