...
* 'umi3.fastq': FASTQ file identical to the above but with the UMI
  extracted and concatenated to the header, with a "_"
  delimiter.
...
* 'multiplex_umi_barcode_adaptor.fastq': FASTQ file with 90 reads:
    ...
    - There are 9 reads with barcode 'TTT', which has a mismatch of
      3nts to 'ACG', 'GAC', 'CGA'. When the file is demultiplexed,
      assuming up to 2 mismatches are allowed, then these 9 reads will
      be unassigned.
