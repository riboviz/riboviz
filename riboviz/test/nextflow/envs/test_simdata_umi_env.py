...
from riboviz.test.nextflow.envs import environment_vars_fixture
from riboviz.test.nextflow.envs import nextflow_fixture
from riboviz.test.nextflow.envs import test_dir_fixture
from riboviz.test.nextflow.envs import workflow_config_fixture

SCENARIOS = [
    ('enviroment-paths', {'use_environment_variables': True}),
    ('relative-paths', {'use_environment_variables': False})
]
""" Test scenarios. """

# TODO update constant
# TEST_CONFIG_FILE = riboviz.test.SIMDATA_UMI_CONFIG
TEST_CONFIG_FILE = os.path.join(test.VIGNETTE_DIR,
                                "simdata_umi_config_env.yaml")
"""
...
@pytest.mark.usefixtures("nextflow")
def test_adaptor_trimming(workflow_config, sample_id):
    """
...
@pytest.mark.parametrize("sample_id", [riboviz.test.SIMDATA_UMI_SAMPLE])
@pytest.mark.usefixtures("nextflow")
def xxx_test_umi_extract(configuration_module, sample_id):
    """
...
@pytest.mark.parametrize("sample_id", [riboviz.test.SIMDATA_UMI_SAMPLE])
@pytest.mark.usefixtures("nextflow")
def xxx_test_umi_group(configuration_module, sample_id):
    """
...
@pytest.mark.parametrize("stats_file", ["edit_distance.tsv",
                                        ...
                                        "per_umi.tsv"])
def xxx_test_umi_stats_files(configuration_module, sample_id, stats_file):
    """
...
@pytest.mark.parametrize("sample_id", [riboviz.test.SIMDATA_UMI_SAMPLE])
@pytest.mark.usefixtures("nextflow")
def xxx_test_tpms_collated_tsv(configuration_module, sample_id):
    """
