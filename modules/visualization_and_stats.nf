...
process generateStatsFigs {
    ...
    input:
        tuple val(sample_id), file(sample_h5), file("${sample_id}.h5.*")
        each file(orf_fasta)
        each file(orf_gff)
        each file(t_rna_tsv)
        each file(codon_positions_rdata)
        each file(features_tsv)
        each file(asite_disp_length_txt)
    output:
        val sample_id, emit: finished_sample_id
        tuple val(sample_id), file("ORF_TPMs_and_counts.tsv"), emit: orf_tpms_and_counts_tsv
        ...
        tuple val(sample_id), file("normalized_density_APEsites_per_codon.pdf") \
            ,emit: normalized_density_apesites_per_codon_pdf, optional: (! (is_t_rna_and_codon_positions_file && params.output_pdfs))
        tuple val(sample_id), file("normalized_density_APEsites_per_codon.tsv") \
            , emit: normalized_density_apesites_per_codon_tsv, optional: (! is_t_rna_and_codon_positions_file)
        tuple val(sample_id), file("normalized_density_APEsites_per_codon_long.tsv") \
            ,emit: normalized_density_apesites_per_codon_long_tsv, optional: (! is_t_rna_and_codon_positions_file)          
        tuple val(sample_id), file("ORF_TPMs_vs_features.pdf") \
            , emit: ORF_TPMs_vs_features_pdf, optional: (! (is_features_file && params.output_pdfs))
        tuple val(sample_id), file("ORF_TPMs_vs_features.tsv") \
            , emit: ORF_TPMs_vs_features_tsv, optional: (! is_features_file)
        tuple val(sample_id), file("read_frame_per_ORF.tsv") \
            , emit: read_frame_per_orf_tsv, optional: (! is_asite_disp_length_file)
        tuple val(sample_id), file("read_frame_per_ORF_filtered.tsv") \
            , emit: read_frame_per_orf_filtered_tsv, optional: (! is_asite_disp_length_file)
        tuple val(sample_id), file("frame_proportions_per_ORF.pdf")\
            , emit: frame_proportions_per_orf_pdf, optional: (! (is_asite_disp_length_file && params.output_pdfs))
    shell:
...
process renameTpms {
    ...
    errorStrategy 'ignore'
    input:
        tuple val(sample_id), file(orf_tpms_and_counts_tsv)
    output:
        val(sample_id), emit: tpms_sample_id
        file "${sample_id}_tpms.tsv", emit: tpms_sample_tsv
    shell:
        """
...
process collateTpms {
    ...
    input:
        val sample_ids
        file orf_tpms_and_counts_tsvs
    output:
        file "TPMs_all_CDS_all_samples.tsv", emit: tpms_all_cds_all_samples_tsv
...
process createVizParamsConfigFile {
    input:
      val viz_params_yaml
     output:
      file "config.yaml", emit: viz_params_config_file_yaml
    shell:
      """
...
process staticHTML {
    ...
    input:
      ...
      tuple val(sample_id), \
        ...
        file(sample_normalized_density_apesites_per_codon_long_tsv) from generate_stats_figs_static_html
    output:
      val sample_id, emit: static_html_sample_ids
      val sample_id, emit: finished_viz_sample_id
      file "${sample_id}_output_report.html", emit: static_html_html
...
process createInteractiveVizParamsConfigFile {
    ...
    input:
     ...
     output:
      file "interactive_viz_config.yaml", emit: interactive_viz_params_config_file_yaml
    shell:
      """
